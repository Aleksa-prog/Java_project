...
public class ClientView extends JFrame {
    ...
    public ClientView(String id, Socket s) throws IOException { // constructor call, it will initialize required variables
        initialize(); // initilize UI components
        this.id = id;
        try {
            file = "C://Users//User//Desktop//Функ.Прог//proj_of_mine_2//" + id + ".txt"; //if there is a file with user's id , then print in on a client messageboard
            if((new File(file)).exists()){
                FileReader reader = new FileReader(file);
                BufferedReader br = new BufferedReader(reader);
                clientMessageBoard.read( br, null );
                br.close();
                clientMessageBoard.requestFocus();}
            else{//if there is no such file create it
                File f = new File("C://Users//User//Desktop//Функ.Прог//proj_of_mine_2//" + id + ".txt");
                f.createNewFile();
            }
        }
                    ...
                    if (m.contains(":;.,/=")) {
                        ...
                        while (st.hasMoreTokens()) {
                            ...
                            if (!id.equals(u)) // we do not need to show own user id in the active user list pane
                                dm.addElement(u); // add all the active user ids to the defaultList to display on active user pane on client view
                        }
                    } else {
    ...
    private void initialize() throws IOException { // initialize all the components of UI
        frame = new JFrame();
        ...
        scrollPane.getViewport().add(clientMessageBoard);
                    ...
                    try {
                        ...
                        if (cast.equalsIgnoreCase("multicast")) {
                            if (flag == 1) { // for multicast check if no user was selected then prompt a message dialog
                                ...
                                clientTypingBoard.setText("");//show the sent message to the sender's message board
                                Date date = new Date();//the message will appear in next way
                                clientMessageBoard.append("<" + id + " to " + clientIds + " || " + date + ">" + ":" + "\n" + textAreaMessage + "\n"+ "\n");
                                String read21 = clientMessageBoard.getText();//read from the client message board
                                PrintWriter writer21 = new PrintWriter(new BufferedWriter(new FileWriter("C://Users//User//Desktop//Функ.Прог//proj_of_mine_2//" + id + ".txt")));
                                writer21.print(read21);//write it into a file with particular id for particular client messagestore
                                writer21.flush();
                                writer21.close();
                            }
                            ...
                            Date date = new Date();
                            clientMessageBoard.append("<" + id + " to All, " + date + " >" + ":" + "\n" + textAreaMessage + "\n" + "\n");
                            PrintWriter writer1 = new PrintWriter(new BufferedWriter(new FileWriter("C://Users//User//Desktop//Функ.Прог//proj_of_mine_2//GroupChat.txt", true)));
                            writer1.println(("<" + id + " to All || " + date + " >" + ": " + "\n" + textAreaMessage.toString() + "\n"));
                            writer1.flush();
                            writer1.close();
                        }
        ...
        clientKillProcessBtn.addActionListener(new ActionListener() { // kill process event
            public void actionPerformed(ActionEvent e) {
                try {
                    ...
                    String read22 = clientMessageBoard.getText();//if client killed process (disconnected) then rewrite the id file
                    PrintWriter writer22 = new PrintWriter(new BufferedWriter(new FileWriter("C://Users//User//Desktop//Функ.Прог//proj_of_mine_2//" + id + ".txt")));
                    writer22.print(read22);
                    writer22.flush();
                    writer22.close();
