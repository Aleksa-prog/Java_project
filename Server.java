...
public class Server {
    ...
    private static int port = 1041;  // port number to be used
            ...
            while (true) {
                try {
                    ...
                    if (activeUserSet != null && activeUserSet.contains(uName)) { // if username is in use then we need to prompt user to enter new name
                        ...
                        PrintWriter writer2 = new PrintWriter(new BufferedWriter(new FileWriter("C://Users//User//Desktop//Функ.Прог//proj_of_mine_2//Users.txt", true)));
                        writer2.println((uName));
                        writer2.flush();
                        writer2.close();
        ...
        public void run() {
            while (allUserNameList != null && !allUsersList.isEmpty()) {  // if allUserList is not empty then proceed further
                try {
                    ...
                    String[] msgList = message.split(":"); // my identifier to identify what action to take on the received message from client actionToBeTaken:clients_for_receiving_msg:message
